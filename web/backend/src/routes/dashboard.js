...
router.get('/', (_req, res) => {
  ...
  const todayIn = db.prepare(`
    SELECT COALESCE(SUM(total_amount),0) as v FROM stock_movements
  ...
  const monthIn = db.prepare(`
    SELECT COALESCE(SUM(total_amount),0) as v FROM stock_movements
    WHERE type='in' AND strftime('%Y-%m',created_at)=strftime('%Y-%m','now')
  `).get().v;

  const monthOut = db.prepare(`
    SELECT COALESCE(SUM(total_amount),0) as v FROM stock_movements
    WHERE type='out' AND strftime('%Y-%m',created_at)=strftime('%Y-%m','now')
  `).get().v;

  // Son 5 hareket
  const recentMovements = db.prepare(`
    SELECT m.id, m.type, m.quantity, m.total_amount, m.created_at, m.notes,
  ...
  const criticalProducts = db.prepare(`
    SELECT p.id, p.sku, p.name, p.quantity, p.min_threshold, p.unit, c.name as category
    FROM products p
    LEFT JOIN categories c ON c.id=p.category_id
    WHERE p.active=1 AND p.quantity <= p.min_threshold
  ...
  const weeklyChart = db.prepare(`
    SELECT DATE(created_at) as date,
           SUM(CASE WHEN type='in'  THEN COALESCE(total_amount,0) ELSE 0 END) as income,
           SUM(CASE WHEN type='out' THEN COALESCE(total_amount,0) ELSE 0 END) as expense
  ...
  const categoryChart = db.prepare(`
  ...
  res.json({
    success: true,
    data: {
      stats: { totalProducts, criticalCount, outOfStock, totalStockCost, totalStockSell,
                profit: totalStockSell - totalStockCost,
