...
router.get('/', (req, res) => {
  ...
  let sql = `
    SELECT m.*, p.name as product_name, p.unit, p.sku
...
router.post('/', (req, res, next) => {
  ...
  if (!product_id || !type || !quantity) {
    const e = new Error('Ürün, tip ve miktar zorunludur'); e.status = 400; return next(e);
  }
  if (!['in','out','return','adjustment','transfer'].includes(type)) {
    const e = new Error('Geçersiz hareket tipi'); e.status = 400; return next(e);
  }

  const product = db.prepare('SELECT id, quantity, buy_price, sell_price FROM products WHERE id=? AND active=1').get(product_id);
  if (!product) { const e = new Error('Ürün bulunamadı'); e.status = 404; return next(e); }
  ...
  if (type === 'in' || type === 'return') {
    ...
    if (qty > prev) {
      const e = new Error(`Yetersiz stok. Mevcut: ${prev}`); e.status = 400; return next(e);
    }
    newQty = prev - qty;
  } else if (type === 'adjustment') {
    newQty = qty; // direkt miktar set et
  } else {
    newQty = prev - qty;
  }
  ...
  const insertMovement = db.transaction(() => {
    db.prepare(`
      INSERT INTO stock_movements
        (id,product_id,type,quantity,prev_quantity,new_quantity,unit_price,total_amount,document_no,notes,user_name)
      VALUES (?,?,?,?,?,?,?,?,?,?,?)
    `).run(id, product_id, type, qty, prev, newQty, price||null, total||null,
           document_no||null, notes||null, user_name);
  ...
  const created = db.prepare(`
    ...
    FROM stock_movements m JOIN products p ON p.id=m.product_id WHERE m.id=?
  `).get(id);
